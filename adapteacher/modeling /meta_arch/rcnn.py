...
class DAobjTwoStagePseudoLabGeneralizedRCNN(GeneralizedRCNN):
    ...
    def forward(
        self, batched_inputs, batched_inputs_add=None, branch="supervised", given_proposals=None, val_mode=False, proposal_index=None
    ):
        """
        Args:
        ...
        # self.D_img.eval()
        images = self.preprocess_image(batched_inputs)
        images_add = self.preprocess_image(batched_inputs_add)

        if "instances" in batched_inputs[0]:
            gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
        else:
            gt_instances = None
        #images.shape == torch.Size([8, 3, 600, 1200])
        features = self.backbone(images.tensor)
        features_add = self.backbone(images_add.tensor)
        ...
        if branch.startswith("supervised"):
            ...
            return {}, proposals_rpn, proposals_roih, ROI_predictions, features, features_add
            ...
            return proposal_losses,proposals_into_roih, proposals_rpn, proposals_roih, proposal_index, features, features_add
    ...
    def visualize_training(self, batched_inputs, proposals, branch=""):
        ...
        for input, prop in zip(batched_inputs, proposals):
            ...
            cv2.imwrite(save_path, vis_img.transpose(1, 2, 0)[:, :, ::-1])  # RGB to BGR
